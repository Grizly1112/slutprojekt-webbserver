...
export default function TermsService(){
    ...
    const Titles = ({ id, title }) => {
        ...
        return (
          <div className="scrollTitle" onClick={handleClick}>
            {title}
          </div>
        );
    };
    
    const TitlesText = ({ id, title, text }) => {
        const [show, setShow] = useState(false);

        return(
            <>
            <div className="containerToS" onClick={() => setShow(!show)}>
                <div className="titleToS" id={id}>{title}</div>
                {show ? <FaAngleUp/>  : <FaAngleDown/>}
            </div>
            {show &&  <div className="textToS">{text}</div>}
            </>
        )
    } 
        ...
        <div className="termsofService">
            <div className="ToSContent">
                ...
                <div className="ToSTextContent">
                    {text.map((message) => (
                        <TitlesText key={message.id} {...message} />
                    ))}
                </div>
            </div>
        </div>
